...
func main() {
	...
	// 发起 mpush 请求
	pairs := make([]*pb2.Pair, 100)
	for i := 0; i < 100; i++ {
		pairs[i] = &pb2.Pair{Key: []byte("k" + fmt.Sprint(i)), Value: []byte("v" + fmt.Sprint(i+1))}
	}
	mpushResponse, err := c.MPush(context.Background(),
		&pb2.MPushRequest{Key: []byte("h"), Pairs: pairs})
	log.Printf("mpushResponse: %+v, err: %+v", mpushResponse, err)

	mmembersResponse, _ := c.MMembers(context.Background(),
		&pb2.MMembersRequest{Key: []byte("h")})
