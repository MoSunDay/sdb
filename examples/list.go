...
func main() {
	...
	lpushResponse, _ := c.LRPush(context.Background(),
		&pb2.LRPushRequest{Key: []byte("h"), Values: values})
	...
	llpushResponse, _ := c.LLPush(context.Background(),
		&pb2.LLPushRequest{Key: []byte("h"), Values: values})
	...
	lmembersResponse, _ := c.LMembers(context.Background(),
		&pb2.LMembersRequest{Key: []byte("h")})
	...
	lpopResponse, err := c.LPop(context.Background(),
		&pb2.LPopRequest{Key: []byte("h"), Values: values})
	log.Printf("lpopResponse: %+v, err: %+v", lpopResponse, err)

	// 发起 lrange 请求
	// 反向检索
	lrangeResponse, err := c.LRange(context.Background(),
		&pb2.LRangeRequest{Key: []byte("h"), Offset: -1, Limit: 10})
	log.Printf("lrangeResponse: %+v, err: %+v", lrangeResponse, err)
	lrangeResponse, err = c.LRange(context.Background(),
		&pb2.LRangeRequest{Key: []byte("h"), Offset: -3, Limit: 3})
	log.Printf("lrangeResponse: %+v, err: %+v", lrangeResponse, err)
	lrangeResponse, err = c.LRange(context.Background(),
		&pb2.LRangeRequest{Key: []byte("h"), Offset: -4, Limit: 3})
	log.Printf("lrangeResponse: %+v, err: %+v", lrangeResponse, err)
	// 正向检索
	lrangeResponse, err = c.LRange(context.Background(),
		&pb2.LRangeRequest{Key: []byte("h"), Offset: 1, Limit: 3})
	log.Printf("lrangeResponse: %+v, err: %+v", lrangeResponse, err)
	lrangeResponse, err = c.LRange(context.Background(),
		&pb2.LRangeRequest{Key: []byte("h"), Offset: 2, Limit: 10})
	log.Printf("lrangeResponse: %+v, err: %+v", lrangeResponse, err)
	lrangeResponse, err = c.LRange(context.Background(),
		&pb2.LRangeRequest{Key: []byte("h"), Offset: 0, Limit: 10})
	log.Printf("lrangeResponse: %+v, err: %+v", lrangeResponse, err)

	// 发起 lexist 请求
	lexistResponse, err := c.LExist(context.Background(),
		&pb2.LExistRequest{Key: []byte("h"), Values: [][]byte{[]byte("h1"),
